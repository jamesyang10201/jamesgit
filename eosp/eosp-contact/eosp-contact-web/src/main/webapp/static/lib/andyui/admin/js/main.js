(function ($) {
    ...
    $.fn.an_main = function (option) {
        ...
        getFistForNav("#" + _stringid.topnavid, function (data) {
            ...
            if (_options.isBaseTopMenu) {
                ...
                appendIn("#" + _stringid.topnavid, "<ul class='"+_stringclass.topnav+"'></ul>");
                createTopMenu();
            }
            ...
            if(_options.menuInTop){
                ...
                if(_options.menuInRight){
                    ...
                    if(menuData.children){
                        ...
                        $(rightMenu).show()
                        removeRightUl();
                        createRightMenu(menuData.children);
                    }else{
                        var rightMenu = "#" + _stringid.menurightid;
                        $(rightMenu).hide()
                    }
    ...
    forRightTree = function(jsonData){
        str += "<ul class='u-down-menu' style = 'width:200px' combo = 'nav1'>";
        for (var i = 0; i < jsonData.length; i++) {
            var urlstr = "";
            try {
                var tar = jsonData[i];
    ...
    // 创建右侧菜单父级对象
    var createRightMenuForParent = function(){
        var menuRightDiv = "<div style='position:absolute;"+
         "right:8px; top:4px; z-index:999'>"+
         "<div class='u-btn-eject' id = 'menu-right' an-combo> "+
        ...
        _mainCenter.prepend(menuRightDiv);
        $("body").after(menuRightScript);
    };

    // 创建右侧菜单
    var createRightMenu = function(data){
        var menuRight = "#" + _stringid.menurightid;
        // var menuNav = "#" + _stringid.menunavid;
        var thisStr = "";
        if(data){
            thisStr = forRightTree(data);
        };
        str = "";
